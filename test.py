...
.fire {
    ...
    width: 50px;
    height: 100%;
    background: linear-gradient(to top, rgba(255,140,0,0.8), rgba(255,69,0,0.0));
...
@keyframes flicker {
    ...
    25% { transform: scaleY(1.2) translateX(-5px); opacity: 0.8; }
    50% { transform: scaleY(0.8) translateX(5px); opacity: 0.6; }
    75% { transform: scaleY(1.1) translateX(-3px); opacity: 0.8; }
...
st.write("✨ 판타지 세계 속, 황금빛 사주풀이로 당신의 운명을 확인하라! ✨")
...
element_report = {
    ...
    "화":{"성격":"열정과 추진력이 폭발하는 자여, 감정을 솔직히 드러내며 카리스마를 뿜어내리라!",
    ...
    "금":{"성격":"냉철하고 결단력 있는 자여, 목표를 명확히 하고 효율적으로 행동하리라!",
    ...
    "수":{"성격":"지혜롭고 신비로운 자여, 직관과 통찰로 운명을 읽으리라!",
